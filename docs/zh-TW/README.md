...
## 為什麼使用 Everything Claude Code？

### 解決的核心問題

| 痛點 | 本專案的解決方案 |
|------|------------------|
| **上下文視窗耗盡** | 提供策略性壓縮技巧、記憶持久化鉤子，讓工作階段持續數小時而非幾分鐘 |
| **重複勞動** | 30+ 個預設指令和技能，一鍵完成常見工作流程 |
| **AI 回覆品質不穩定** | 規則系統確保程式碼風格一致、安全合規、測試覆蓋 |
| **Token 成本高昂** | 子代理程式架構 + 模型選擇策略，成本降低 50-70% |
| **跨工作階段遺忘** | 持續學習系統自動擷取模式，下次工作階段自動套用 |
| **手動設定繁瑣** | 一鍵安裝外掛程式，立即獲得完整設定 |

### 誰適合使用？

- **日常使用 Claude Code 的開發者**：節省設定時間，專注於編碼
- **團隊技術負責人**：標準化團隊的 AI 輔助開發流程
- **追求效率的工程師**：將 AI 程式設計效率提升 2-5 倍
- **多語言開發者**：支援 TypeScript、Go、C++、Qt、Python 等
...
| 指標 | 提升幅度 |
|------|----------|
| 單次工作階段有效編碼時間 | 3-5 倍延長 |
| 重複性任務時間 | 減少 80% |
| 程式碼審查覆蓋率 | 從人工 30% 提升到 AI+人工 90% |
| Token 消耗 | 使用 mgrep + 子代理程式減少 50% |
...
|------|------|----------|
| `/plan` | 規劃功能實作 | 開始新功能前 |
| `/tdd` | 測試驅動開發 | 撰寫新功能或修 Bug |
| `/code-review` | 程式碼審查 | 提交前檢查 |
| `/build-fix` | 修復建置錯誤 | 編譯失敗時 |
| `/refactor-clean` | 清理無用程式碼 | 重構後 |
...
不要讓主 Claude 做所有事情。委派給專門的子代理程式：

```markdown
# 情境：需要實作一個新功能

1. 使用 planner 代理程式規劃實作步驟
2. 使用 tdd-guide 代理程式撰寫測試和程式碼
3. 使用 code-reviewer 代理程式審查程式碼
4. 使用 security-reviewer 代理程式檢查安全問題
```

**效果**：每個代理程式專注單一職責，結果更準確，上下文消耗更少。

### 2. 管理上下文視窗

**黃金法則**：200k 上下文 = 珍貴資源，不要浪費

| 策略 | 具體做法 |
|------|----------|
| **停用未用 MCP** | `/mcp` 查看，保持 <10 個啟用 |
| **使用 mgrep** | 比 grep 節省 50% token |
| **策略性壓縮** | 探索完成後手動 `/compact` |
| **模組化程式碼** | 檔案保持 200-400 行，最多 800 行 |
...
| 鉤子 | 自動執行內容 |
|------|--------------|
| `PostToolUse` | 編輯 TS/JS 後自動 Prettier 格式化 |
| `PostToolUse` | 編輯 TS 後自動型別檢查 |
| `PostToolUse` | 偵測 console.log 並警告 |
| `Stop` | 工作階段結束前稽核所有 console.log |

### 6. 模型選擇策略

| 任務類型 | 建議模型 | 原因 |
|----------|----------|------|
| 檔案搜尋、簡單編輯 | Haiku | 快速便宜 |
| 多檔案實作、PR 審查 | Sonnet | 最佳平衡 |
| 架構決策、安全分析、複雜除錯 | Opus | 深度推理 |

**經驗法則**：90% 任務用 Sonnet，Opus 用於：首次嘗試失敗、跨 5+ 檔案、架構決策、安全關鍵程式碼。
...
| 問題 | 解決方案 |
|------|----------|
| 建置失敗 | `/build-fix` 或使用 `build-error-resolver` 代理程式 |
| 測試失敗 | 使用 `tdd-guide` 代理程式除錯 |
| 上下文快滿了 | `/compact` 手動壓縮，或 `/fork` 分支工作階段 |
| Claude 忘記了之前的決定 | 檢查 `.claude/` 目錄下的工作階段檔案 |

### 程式碼品質檢查清單

每次提交前確保：

- [ ] 無寫死密鑰（API keys、密碼、tokens）
- [ ] 所有使用者輸入已驗證
- [ ] 測試覆蓋率 ≥ 80%
- [ ] 無 console.log 語句
- [ ] 檔案大小 < 800 行
