...
## 为什么使用 Everything Claude Code？

### 解决的核心问题

| 痛点 | 本项目的解决方案 |
|------|------------------|
| **上下文窗口耗尽** | 提供战略性压缩技巧、内存持久化钩子，让会话持续数小时而不是几分钟 |
| **重复劳动** | 30+ 个预配置命令和技能，一键完成常见工作流 |
| **AI 回复质量不稳定** | 规则系统确保代码风格一致、安全合规、测试覆盖 |
| **Token 成本高昂** | 子代理架构 + 模型选择策略，成本降低 50-70% |
| **跨会话遗忘** | 持续学习系统自动提取模式，下次会话自动应用 |
| **手动配置繁琐** | 一键安装插件，即刻获得完整配置 |

### 谁适合使用？

- **日常使用 Claude Code 的开发者**：节省配置时间，专注于编码
- **团队技术负责人**：标准化团队的 AI 辅助开发流程
- **追求效率的工程师**：将 AI 编程效率提升 2-5 倍
- **多语言开发者**：支持 TypeScript、Go、C++、Qt、Python 等
...
| 单次会话有效编码时间 | 3-5 倍延长 |
| 重复性任务时间 | 减少 80% |
| 代码审查覆盖率 | 从人工 30% 提升到 AI+人工 90% |
| Token 消耗 | 使用 mgrep + 子代理减少 50% |
...
|------|------|----------|
| `/plan` | 规划功能实现 | 开始新功能前 |
| `/tdd` | 测试驱动开发 | 编写新功能或修 Bug |
| `/code-review` | 代码审查 | 提交前检查 |
| `/build-fix` | 修复构建错误 | 编译失败时 |
| `/refactor-clean` | 清理死代码 | 重构后 |
...
不要让主 Claude 做所有事情。委托给专门的子代理：

```markdown
# 场景：需要实现一个新功能

1. 使用 planner 代理规划实现步骤
2. 使用 tdd-guide 代理编写测试和代码
3. 使用 code-reviewer 代理审查代码
4. 使用 security-reviewer 代理检查安全问题
```

**效果**：每个代理专注单一职责，结果更准确，上下文消耗更少。

### 2. 管理上下文窗口

**黄金法则**：200k 上下文 = 珍贵资源，不要浪费

| 策略 | 具体做法 |
|------|----------|
| **禁用未用 MCP** | `/mcp` 查看，保持 <10 个启用 |
| **使用 mgrep** | 比 grep 节省 50% token |
| **战略性压缩** | 探索完成后手动 `/compact` |
| **模块化代码** | 文件保持 200-400 行，最多 800 行 |
...
| 钩子 | 自动执行内容 |
|------|--------------|
| `PostToolUse` | 编辑 TS/JS 后自动 Prettier 格式化 |
| `PostToolUse` | 编辑 TS 后自动类型检查 |
| `PostToolUse` | 检测 console.log 并警告 |
| `Stop` | 会话结束前审计所有 console.log |

### 6. 模型选择策略

| 任务类型 | 推荐模型 | 原因 |
|----------|----------|------|
| 文件搜索、简单编辑 | Haiku | 快速便宜 |
| 多文件实现、PR 审查 | Sonnet | 最佳平衡 |
| 架构决策、安全分析、复杂调试 | Opus | 深度推理 |

**经验法则**：90% 任务用 Sonnet，Opus 用于：首次尝试失败、跨 5+ 文件、架构决策、安全关键代码。
...
| 问题 | 解决方案 |
|------|----------|
| 构建失败 | `/build-fix` 或使用 `build-error-resolver` 代理 |
| 测试失败 | 使用 `tdd-guide` 代理调试 |
| 上下文快满了 | `/compact` 手动压缩，或 `/fork` 分支会话 |
| Claude 忘记了之前的决定 | 检查 `.claude/` 目录下的会话文件 |
